...
echo "############################################################"
echo "#                                                          #"
echo "#         Base Install Script for Proxmox LXC              #"
echo "#                                                          #"
...
echo -n "Updating and upgrading system packages..."
{
    apt update && apt upgrade -y && apt install -y \
        ...
        unattended-upgrades apt-listchanges dnsutils cron net-tools tcpdump btop
} &> /dev/null &
show_spinner $!
if [[ $? -eq 0 ]]; then
...
#----------------------------------------------------------------------------
echo -n "Configuring system timezone..."
{
...
if [[ $? -eq 0 ]]; then
    ...
    echo -e "        \xE2\x9D\x8C Error: Timezone configuration failed."
    exit 1
fi
...
if [[ $? -eq 0 ]]; then
    ...
    echo -e "        \xE2\x9D\x8C Error: Vim configuration failed."
    exit 1
fi
